        ...
        <div className='max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12 relative z-10'>
          ...
          <div className='flex-1 space-y-8 relative md:pr-8 lg:pr-19 text-center md:text-left'>
            ...
            <p className='text-lg md:text-lg lg:text-xl font-playfair italic sm:text-xl text-amber-100 max-w-xl opacity-90  mx-auto md:mx-0'>
              Best cooks and best delivery team at your service. Hot, tasty food can reach you in about 60 minutes.
            </p>

            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 text-sm text-amber-200/95">
              <span className="rounded-full border border-amber-500/40 bg-amber-900/30 px-3 py-1 font-cinzel">100% Vegetarian</span>
              <span className="rounded-full border border-amber-500/40 bg-amber-900/30 px-3 py-1 font-cinzel">Fresh ingredients</span>
              <span className="rounded-full border border-amber-500/40 bg-amber-900/30 px-3 py-1 font-cinzel">Easy online order</span>
            </div>
            ...
            {/* <form onSubmit={handleSearch} className='relative max-w-2xl mx-auto md:mx-0 group'>
              <div className='relative flex items-center bg-amber-900/30 rounded-xl border-2 border-amber-500/30 shadow-2xl hover:bg-amber-400/50 transition-all duration-300'>
                ...
                <input
                  type='text' value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder='Discover your next favorite meal...'
            ...
            {/* <div className='flex flex-wrap gap-4 justify-center md:justify-start mt-6'>
              <button className='group flex items-center gap-3 bg-amber-800/30 hover:bg-amber-800/50 px-6 py-3 rounded-xl transition-all duration-300 border-2 border-amber-700/50 hover:boarder-amber-400 backdrop-blur-sm '>
                <FaDownload className='text-xl text-amber-400 group-hover:animate-bounce' />
                <span className='text-lg'>Download App</span>
              </button>

              <button onClick={() => setShowVideo(true)} className='group flex items-center gap-3 bg-gradient-to-r from-amber-400 to bg-amber-300 hover:from-amber-300 hover:to-amber-200 px-6 py-3 rounded-xl transition-all duration-300 shadow-lg hover:shadow-amber-300/30 '>
          ...
          <div className='flex-1 relative group mt-8 md:mt-0 min-h-[300px] sm:min-h-[400px]'>
            ...
            <div className='relative rounded-full p-1 bg-gradient-to-br from-amber-700 via-amber-800 to-amber-400 shadow-2xl z-20 w-[250px] xs:w-[300px] sm:w-[350px] h-[250px] xs:h[300px] sm:h-[350px] mx-auto'>
              <img src={bannerImage} alt="Banner" loading="eager" decoding="async" className='rounded-full border-4 xs:border-8 border-amber-900/50 w-full h-full object-cover object-top' />
            ...
            {orbitImages.map((imgSrc, index) => {
              return (
                <div key={index} className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${index === 0 ? 'orbit' : `orbit-delay-${index * 5}`} w-[80px] xs:w-[100px] sm:w-[150px] h-[80px] xs:h-[100px] sm:h-[150px] `}>
